    ...
    private String handleProductAdding(HttpServletRequest request, HttpServletResponse response) {
        if (AuthUtils.isAdmin(request)) {
            ...
            if(pdao.isProductExists(id)){
                checkError = "Product ID is already exists.";
            }
            if(id == null || id.trim().isEmpty()){
                checkError = "Product id cannot be empty.";
            }
            if(name == null || name.trim().isEmpty()){
                checkError = "Product name cannot be empty.";
            }
            if(image == null || image.trim().isEmpty()){
                checkError = "Product image cannot be empty."; 
            }
            if(price == null || price.trim().isEmpty()){
                checkError = "Product price cannot be empty."; 
            }
            if(description == null || description.trim().isEmpty()){
                checkError = "Product description cannot be empty."; 
            }
            if(size == null || size.trim().isEmpty()){
                checkError = "Product size cannot be empty."; 
            }
            if(status == null || status.trim().isEmpty()){
                checkError = "Product status cannot be empty."; 
            }
            if(price_value < 0){
